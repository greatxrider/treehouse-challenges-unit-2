...
function handlePagination(array) {
  ...
  for (let i = 1; i <= numberOfButtons; i++) {
    ...
    const buttonHtml = `<li><button>${i}</button></li>`;
...
function showPage(array, page) {
  ...
  const startAuthorIndex = (page * authorsPerPage) - authorsPerPage;
  ...
  const lastAuthorIndex = (page * authorsPerPage) - 1;
  ...
  for (let i = 0; i < array.length; i++) {
    ...
    if (i >= startAuthorIndex && i <= lastAuthorIndex) {
      ...
      //      Hint: insertAdjacentHTML()
      authorContainer.insertAdjacentHTML('beforeend', authorCardHtml);
    }
  }
}
